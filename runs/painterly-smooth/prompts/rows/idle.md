Create one horizontal animation strip for Codex pet `apollo-painterly`, state `idle`.
...
Smooth continuous-loop contract: Create six consecutive in-between moments of one seamless cyclic motion, not six distinct poses. Frame 6 must flow directly back into frame 1. Keep the same camera, scale, body position, footing, and limb configuration in every frame; adjacent frames must look nearly identical, as the next instant of the same motion. Change only the stated micro-motion; do not introduce pose changes, scale pops, or baseline shifts.

Pose-family anchor: Use the attached current-pose-family image only to keep Apollo in exactly this sitting-at-rest stance.

Motion brief: Across the six-frame loop, one slow breath only: chest and shoulders rise about 2% by the middle and settle back. Ears relax slightly. Make one soft blink unfold across two adjacent middle frames. Legs, paws, tail, and head position do not move.
