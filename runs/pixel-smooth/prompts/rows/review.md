Create one horizontal animation strip for Codex pet `apollo-pixel`, state `review`.
...
Smooth-loop contract: Make six consecutive in-between moments of one seamless continuous loop, not six distinct poses. Keep identical camera, scale, body placement, footing, planted legs and paws, and silhouette across all frames; adjacent frames must read as the next instant, and frame 6 must flow directly into frame 1. Use the current pose-family anchor exactly: Apollo sits proudly, mouth open and pleased. The tail completes one full wag sweep left to center to right to center across the loop, with only a subtle happy head bob. No other movement.

ANCHOR-LOCK REPAIR PRIORITY: The supplied pose-family anchor visibly shows Apollo standing proudly in front/three-quarter view. Match that standing anchor's camera, leg placement, and silhouette exactly; do not turn Apollo into a seated pose. Keep all paws planted. Make the tail visibly sweep left to center to right to center once across the six frames, with a subtle happy head bob and pleased open mouth.
