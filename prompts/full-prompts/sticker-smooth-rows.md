# Task: Regenerate 4 sticker rows as SMOOTH continuous motion loops
...
This is a PARTIAL run: regenerate ONLY the `idle`, `waiting`, `running`, and `review` rows for the existing pet `apollo-sticker`. Do NOT generate the other five rows, do NOT compose a full atlas, do NOT package.
...
1. Bootstrap a run at `runs/sticker-smooth` with `prepare_pet_run.py`:
   - pet-name "Apollo", pet-id `apollo-sticker`, display-name "Apollo (Sticker)"
   - description: "Apollo, a friendly medium-sized mutt with a white/creme coat, brown saddle patches, and a brown face mask split by a white blaze — sticker edition."
   - style preset `sticker`
   - references: `refs/IMG_4195.png`, `refs/IMG_0887.png`, `refs/IMG_9501.png`, `variants/apollo-sticker.png`
...
2. The canonical base is ALREADY APPROVED: copy `variants/apollo-sticker.png` in as the base job's selected output (per the skill), create `references/canonical-base.png`, mark the base job complete. Do not regenerate the base.
3. Pose-family anchors: the currently shipped cells live in `dist/codex-pets/apollo-sticker/spritesheet.webp` (192×208 cells, rows top-to-bottom: idle, running-right, running-left, waving, jumping, failed, waiting, running, review). Crop frame 0 of each of the four target rows to PNG and attach it to that row's generation job as an extra reference labeled "current pose family — keep this stance". The regenerated row must stay in the same stance family so the pet doesn't change character between states.
...
- Extract only these rows: `extract_strip_frames.py --states idle,waiting,running,review` into `runs/sticker-smooth/frames/`; run `inspect_frames.py` on them. Use `--method stable-slots` (with the matching inspect flag) if per-frame fit causes size popping and the strip itself is stable.
...
- Leave the final keyed 192×208 RGBA frames at `runs/sticker-smooth/frames/<state>/`. Keep the decoded strips. No atlas, no packaging, no cleanup of frames.

## Final response — return exactly:

frames_root=<absolute path to runs/sticker-smooth/frames>
